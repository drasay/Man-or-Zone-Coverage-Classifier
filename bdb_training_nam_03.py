...
class ManZoneNAM(nn.Module):
    def __init__(self, feature_len=5, hidden_dim=64, num_layers=2, dropout=0.1, output_dim=2):
        super(ManZoneNAM, self).__init__()
        self.feature_len = feature_len
        self.nam_subnets = nn.ModuleList([
            self._make_mlp(1, hidden_dim, num_layers, dropout) for _ in range(feature_len)
        ])
        self.output_layer = nn.Linear(feature_len, output_dim)

    def _make_mlp(self, input_dim, hidden_dim, num_layers, dropout):
        layers = [nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout)]
        for _ in range(num_layers - 1):
            layers.extend([nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout)])
        layers.append(nn.Linear(hidden_dim, 1))  # Output a single contribution per feature
        return nn.Sequential(*layers)

    def forward(self, x):
        # x: (batch_size, num_players, feature_len)
        x = x.mean(dim=1)  # Average over players → (batch_size, feature_len)
        feature_outputs = [net(x[:, i:i+1]) for i, net in enumerate(self.nam_subnets)]
        combined = torch.cat(feature_outputs, dim=1)  # (batch_size, feature_len)
...
def objective(trial: Trial, week_eval, device, feature_len=5, output_dim=2, 
    ...
    checkpoint_path = f"{trials_dir}/trial_{trial.number}_week{week_eval}_nam.pth"
...
def run_bayesian_optimization(week_eval, min_trials=5, max_trials=20, convergence_threshold=0.005,
    ...
    if study_name is None:
        study_name = f"man_zone_nam_week{week_eval}"
    ...
    optim_save_path = f"{save_path}/optim_results_nam"
    ...
    final_checkpoint_path = f"C:/python/nfl-big-data-bowl-2025/best_model_week{week_eval}_nam.pth"
