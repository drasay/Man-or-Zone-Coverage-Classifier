...
import pandas as pd
import joblib
from bdb_training_transformers_03 import ManZoneTransformer
...
# Load Model
best_params = joblib.load(f"C:/python/nfl-big-data-bowl-2025/optim_results_transformer/man_zone_transformer_week{week_eval}_study.pkl").best_trial.params
...
model.load_state_dict(torch.load(f"C:/python/nfl-big-data-bowl-2025/best_model_week{week_eval}_transformer.pth", weights_only=True))
model.eval()
...
df_results.to_csv(f"C:/python/nfl-big-data-bowl-2025/week_{week_eval}_preds_transformer.csv", index=False)
