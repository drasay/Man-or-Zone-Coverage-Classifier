...
class ManZoneTransformer(nn.Module):
    def __init__(self, feature_len=5, model_dim=64, num_heads=2, num_layers=4, 
                 dim_feedforward=256, dropout=0.1, output_dim=2):
...
def objective(trial: Trial, week_eval, device, feature_len=5, output_dim=2, 
    ...
    model_dim = trial.suggest_categorical("model_dim", [32, 64, 96, 128])
    num_heads = trial.suggest_categorical("num_heads", [2, 4, 8])
    num_layers = trial.suggest_int("num_layers", 2, 6)
    dropout = trial.suggest_float("dropout", 0.1, 0.5)
    
    # Ensure num_heads divides model_dim evenly
    while model_dim % num_heads != 0:
        num_heads = trial.suggest_categorical("num_heads", [h for h in [2, 4, 8] if model_dim % h == 0])
    
    dim_feedforward = model_dim * trial.suggest_categorical("dim_feedforward_multiplier", [2, 4, 8])
    ...
    checkpoint_path = f"{trials_dir}/trial_{trial.number}_week{week_eval}_transformer.pth"
...
def run_bayesian_optimization(week_eval, min_trials=5, max_trials=20, convergence_threshold=0.005,
    ...
    if study_name is None:
        study_name = f"man_zone_transformer_week{week_eval}"
    ...
    optim_save_path = f"{save_path}/optim_results_transformer"
    ...
    model = ManZoneTransformer(
        ...
        dim_feedforward=best_params["model_dim"] * best_params["dim_feedforward_multiplier"],
    ...
    final_checkpoint_path = f"C:/python/nfl-big-data-bowl-2025/best_model_week{week_eval}_transformer.pth"
